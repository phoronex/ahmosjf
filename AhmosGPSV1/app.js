...
const GPSDashboard = (function() {
    ...
    function processDeviceData(jsonData) {
        ...
        Object.entries(jsonData.devices).forEach(([deviceId, deviceData], index) => {
        ...
        // Force render charts for expanded devices after render
        setTimeout(() => {
            Object.keys(appState.devices).forEach((deviceId, index) => {
                if (index < 2 && appState.expandedDevices.has(deviceId)) {
    ...
    function createDeviceContent(deviceId, device) {
        ...
        return `
            ...
            <div class="device-view-controls">
                ...
                <button class="view-btn export-btn" 
                        onclick="GPSDashboard.exportDeviceData('${deviceId}')">
                    <span>📥</span> Export
                </button>
            </div>
    ...
    return {
        ...
        toggleDevice: function(deviceId) {
            ...
            // If expanding, immediately render chart or table
            if (appState.expandedDevices.has(deviceId)) {
                const viewMode = appState.viewModes.get(deviceId) || 'graph';
                if (viewMode === 'graph') {
                    // Force canvas resize and chart render
                    setTimeout(() => {
                        const canvas = document.getElementById(`chart_${deviceId}`);
                        if (canvas) {
                            canvas.width = canvas.offsetWidth;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        if (Object.keys(GPSDashboard.appState.devices).length === 0) {
            ...
            // If data already exists (from saved state), render charts for expanded devices
            setTimeout(() => {
                Object.keys(GPSDashboard.appState.devices).forEach((deviceId) => {
                    if (GPSDashboard.appState.expandedDevices.has(deviceId)) {
                        const viewMode = GPSDashboard.appState.viewModes.get(deviceId) || 'graph';
                        if (viewMode === 'graph') {
                            setTimeout(() => {
                                const canvas = document.getElementById(`chart_${deviceId}`);
                                if (canvas) {
                                    ...
                                    if (GPSDashboard.appState.charts.has(deviceId)) {
                                        GPSDashboard.appState.charts.get(deviceId).destroy();
                                    }
